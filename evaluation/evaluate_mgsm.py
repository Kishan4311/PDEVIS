...
# coding=utf-8
"""
python /home/iitb/Kishan_SpecDec/_spade2/evaluation/mgsm/evaluate_mgsm.py \
  --draft_device "cuda:0" \
  --target_device "cuda:3" \
  --dataset "/home/iitb/Kishan_SpecDec/_spade2/Data/mgsm.json" \
  ...
  --output "/home/iitb/Kishan_SpecDec/_spade2/evaluation/mgsm/llama_k40_mgsm_result_point9_gm6_3B_70B_genLen8.json"
...
def load_mgsm(path: str, k: int):
    with open(path, "r") as f:
        data = json.load(f)
    return data[:k]

def format_mgsm_prompt(problem: str) -> str:
    return (
...
def main(args):
    ...
    dataset = load_mgsm(args.dataset, args.k)
    # results = []
    ...
    for item in tqdm(dataset, desc="Evaluating MGSM"):
        qid = item["question_id"]
        prompt = item["prompt"]
        # raw_prompt = item["prompt"]
        # prompt = format_mgsm_prompt(raw_prompt)
        label = item["label"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("MGSM Evaluation with SPADE")
